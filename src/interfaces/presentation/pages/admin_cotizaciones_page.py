...
class AdminCotizacionesPage:
    ...
    def render(self):
        ...
        with content:
            ...
            ui.button("Exportar Excel", on_click=self.exportar_excel).classes("av-btn-p").style("margin-bottom:18px;")
            with ui.card().style(
                "background:#141414;border:1px solid #242424;border-radius:22px;"
                "padding:26px;color:#fff;width:100%;"
            ):
                ui.html(f'<div style="display:flex;justify-content:space-between;gap:14px;align-items:center;margin-bottom:18px;flex-wrap:wrap;"><h2 style="font-size:20px;color:#fff;margin:0;">Listado de cotizaciones</h2><span style="background:rgba(230,57,70,.12);border:1px solid rgba(230,57,70,.45);color:#fff;border-radius:999px;padding:8px 12px;font-size:12px;font-weight:900;">{len(self.cotizaciones)} registros</span></div>')
