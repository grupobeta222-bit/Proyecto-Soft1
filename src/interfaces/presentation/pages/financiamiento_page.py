...
def financiamiento():
    ...
    with ui.element("div").style("padding-top:110px;"):
        with ui.element("div").classes("av-section"):
            ...
            ui.html('<p style="font-size:15px;color:#777;line-height:1.7;margin-top:-24px;margin-bottom:32px;max-width:680px;">Compara los planes disponibles y elige el que mejor se ajuste a tu presupuesto. Luego puedes usar el cotizador para calcular tu cuota mensual.</p>')

            with ui.element("div").classes("av-finance-grid"):
                for plan in planes:
                    mostrar_plan(plan)

            ui.button("Calcular una cotización", on_click=ir_cotizar).classes("av-btn-p").style("margin-top:32px;")
