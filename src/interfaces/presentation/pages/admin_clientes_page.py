...
def admin_clientes():
    ...
    with content:
        ...
        with ui.card().style('background:#141414;border:1px solid #242424;border-radius:22px;padding:26px;color:#fff;width:100%;'):
            ...
            filas = ""
            for r in rows:
                filas += f"<tr><td>{r['nombre']}</td><td>{r['cedula']}</td><td>{r['email']}</td><td>{r['telefono']}</td><td>{r['ciudad']}</td><td>{r['cotizaciones']}</td></tr>"
            ui.html(f'<div style="overflow-x:auto;width:100%;"><table class="av-admin-table"><thead><tr><th>Nombre</th><th>Cédula</th><th>Email</th><th>Teléfono</th><th>Ciudad</th><th>Cotizaciones</th></tr></thead><tbody>{filas}</tbody></table></div>')
