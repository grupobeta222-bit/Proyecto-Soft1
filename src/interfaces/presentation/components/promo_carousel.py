...
def render_promo_carousel(vehiculos):
    ...
    with ui.element("div").style("background:#0f0f0f;padding:80px 0;"):
        with ui.element("div").classes("av-section"):
            ...
            with ui.carousel(animated=True, arrows=True, navigation=True).props("height=360px autoplay").classes("w-full"):
                for vehiculo in slides:
                    with ui.carousel_slide().style("background:#141414;border:1px solid #242424;border-radius:22px;padding:28px;"):
                        with ui.row().style("height:100%;align-items:center;justify-content:space-between;gap:30px;"):
                            with ui.column().style("gap:10px;max-width:430px;"):
                                ...
                                ui.button("Cotizar promoción", on_click=boton_cotizar(vehiculo.id)).classes("av-btn-p").style("width:max-content;margin-top:10px;")
                            ui.html(f'<img src="{car_image_src(vehiculo)}" alt="{vehiculo.marca} {vehiculo.modelo}" style="max-width:420px;width:45%;height:260px;object-fit:contain;filter:drop-shadow(0 25px 38px rgba(0,0,0,.45));">')
